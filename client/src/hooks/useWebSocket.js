    ...
    wsRef.current.onmessage = (event) => {
      ...
      switch (messageData.type) {
        ...
        case MESSAGE_TYPES.USER_INFO_UPDATE_ERROR:
          ...
          window.dispatchEvent(
            new CustomEvent("userInfoUpdateError", {
          ...
          if (messageData.users) {
            ...
            window.dispatchEvent(
              new CustomEvent("localStorageUpdate", {
                detail: {
                  key: "users",
                  newValue: JSON.stringify(messageData.users),
                },
              })
            );
          }
        ...
        case MESSAGE_TYPES.FRIEND_INVITATION_RECEIVED:
          ...
          const receivedInvitations = JSON.parse(
            localStorage.getItem("receivedInvitations") || "[]"
          );
          ...
          window.dispatchEvent(
            new CustomEvent("friendInvitationReceived", {
        ...
        case MESSAGE_TYPES.FRIEND_INVITATION_SENT:
          ...
          if (messageData.success && messageData.invitation) {
            const sentInvitations = JSON.parse(
              localStorage.getItem("sentInvitations") || "[]"
            );
            sentInvitations.push(messageData.invitation);
            localStorage.setItem(
              "sentInvitations",
              JSON.stringify(sentInvitations)
            );
          }
          // Trigger custom event for friend invitation sent confirmation
          window.dispatchEvent(
            new CustomEvent("friendInvitationSent", {
              detail: {
                success: messageData.success,
                error: messageData.error,
                toUserId: messageData.toUserId,
                invitation: messageData.invitation,
              },
            })
          );
          break;
        case MESSAGE_TYPES.FRIEND_INVITATION_ACCEPTED:
          ...
          const updatedSentInvitations = sentInvitations.filter(
            (inv) => inv.id !== messageData.invitationId
          );
          localStorage.setItem(
            "sentInvitations",
          ...
          window.dispatchEvent(
            new CustomEvent("friendInvitationAccepted", {
              detail: {
                invitationId: messageData.invitationId,
                acceptedBy: messageData.acceptedBy,
              },
            })
          );
          break;
        case MESSAGE_TYPES.FRIEND_INVITATION_DECLINED:
          // Remove invitation from sent invitations if it exists
          const sentInvitations2 = JSON.parse(
            localStorage.getItem("sentInvitations") || "[]"
          );
          const updatedSentInvitations2 = sentInvitations2.filter(
            (inv) => inv.id !== messageData.invitationId
          );
          localStorage.setItem(
            "sentInvitations",
            JSON.stringify(updatedSentInvitations2)
          );
          // Trigger custom event for friend invitation declined
          window.dispatchEvent(
            new CustomEvent("friendInvitationDeclined", {
              detail: {
                invitationId: messageData.invitationId,
                declinedBy: messageData.declinedBy,
              },
            })
          );
          break;
        case MESSAGE_TYPES.FRIEND_INVITATION_CANCELLED:
          ...
          const receivedInvitations2 = JSON.parse(
            localStorage.getItem("receivedInvitations") || "[]"
          );
          const updatedReceivedInvitations = receivedInvitations2.filter(
            (inv) => inv.id !== messageData.invitationId
          );
          localStorage.setItem(
            "receivedInvitations",
            JSON.stringify(updatedReceivedInvitations)
          );
          // Trigger custom event for friend invitation cancelled
          window.dispatchEvent(
            new CustomEvent("friendInvitationCancelled", {
              detail: {
                invitationId: messageData.invitationId,
                cancelledBy: messageData.cancelledBy,
              },
            })
          );
          break;
        ...
        case MESSAGE_TYPES.FRIENDS_LIST_SYNC:
          ...
          window.dispatchEvent(
            new CustomEvent("friendsListSync", {
        ...
        case MESSAGE_TYPES.PRIVATE_CHAT_CREATED:
          ...
          window.dispatchEvent(
            new CustomEvent("privateChatCreated", {
